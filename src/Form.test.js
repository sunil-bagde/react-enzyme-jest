...
describe("<Form />", () => {
    test("receive ", () => {
        ...
        const promotionInput = wrapper.find(testId("checked"));
        const checker = promotionInput.props().checked
    ...
    test("allows user to fill form", () => {
        ...
        const nameInput = updateInput(wrapper, testId("name"), 'admin')

        expect(nameInput.props().value).toBe('admin')

        const emailInput = updateInput(wrapper, testId("email"), 'admin@gmail.com')
        const numberInput = updateInput(wrapper, testId("number"), '8655062213')
        wrapper.find(testId("checked")).simulate('click')

        expect(nameInput.props().value).toBe('admin')
        expect(emailInput.props().value).toBe('admin@gmail.com')
        expect(numberInput.props().value).toBe('8655062213')
        expect(wrapper.find(testId("checked")).props().checked).toBe(false)
